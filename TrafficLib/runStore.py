...
import sys
sys.path.append("/root/work/TRAFFIC/Traffic/TrafficLib/")
...
def read_training(train_dir, num_landmarks, num_points, landmarks, curvature, torsion):
    ...
    # print "Data Set : ",num_fib, num_feat, num_pt
    # Shuffle data
...
def run_store(train_dir='', test_dir='', valid_dir='', original_dir='', num_landmarks=5, num_points=50, lmOn=True, curvOn=True, torsOn=True):
    if train_dir:
        dataset, labels = read_training(train_dir, num_landmarks, num_points, lmOn, curvOn, torsOn)

        #   Convert to Examples and write the result to TFRecords.
        convert_to(dataset, labels, 'train', train_dir)

    elif valid_dir:
        dataset, labels = read_training(valid_dir, num_landmarks, num_points, lmOn, curvOn, torsOn)

        #   Convert to Examples and write the result to TFRecords.
        convert_to(dataset, labels, 'valid', valid_dir)

    elif test_dir:
        dataset, labels = read_testing(test_dir, original_dir, num_landmarks, num_points, lmOn, curvOn, torsOn)

        # Convert to Examples and write the result to TFRecords.
        convert_to(dataset, labels, 'test', test_dir)
        # Convert_to(test_set.validation, 'validation')


def main(_):
    run_store(FLAGS.train_dir, FLAGS.test_dir, FLAGS.valid_dir, FLAGS.original_dir, FLAGS.num_landmarks, FLAGS.num_points, FLAGS.landmarks, FLAGS.curvature, FLAGS.torsion)
  # Get the data.
