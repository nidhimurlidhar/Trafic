...
def run_pipeline_train(data_dir, num_epochs, lr, num_lm, summary_dir, checkpoint_dir):
	...
	if num_lm==5:
		run_training(data_dir=data_dir, summary_dir=summary_dir,num_epochs=num_epochs, learning_rate=lr, multiclass=False, checkpoint_dir=checkpoint_dir)
	if num_lm==32:
		run_training(data_dir=data_dir, summary_dir=summary_dir,num_epochs=num_epochs, learning_rate=lr, multiclass=True, checkpoint_dir=checkpoint_dir)
...
def main():
	...
	if biclass:
		run_pipeline_train(data_dir, num_epochs, lr, 5, summary_dir, checkpoint_dir)
	elif multiclass:
